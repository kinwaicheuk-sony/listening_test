...
def extract_label(sample_files, occurrence_counter):
    ...
    # Get only the steermusic, textinv, or dreamsound files
    selected_files = [f for f in sample_files if re.search(r"_(steermusic|textinv|dreamsound)\.flac$", f, re.IGNORECASE)]
    ...
    # Extract labels
    match1 = re.search(r"_(steermusic|textinv|dreamsound)\.flac$", sorted_files[0], re.IGNORECASE)
    match2 = re.search(r"_(steermusic|textinv|dreamsound)\.flac$", sorted_files[1], re.IGNORECASE)
...
# Example usage
json_file = "audio_questions2.json"  # Replace with actual JSON file
output_data = generate_ground_truth(json_file)

# Save to a new JSON file
output_file = "ground_truth_with_occurrences_fixed2.json"
